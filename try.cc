...
#include "src/mobility/helper/ns2-mobility-helper.cc"
*/
#include <iostream>
#include <sstream>
#include <fstream>
...
uint16_t maxDistance = 30; // m //最大传播距离
uint32_t packetSize  = 1000; // bytes
uint16_t beaconPort  = 80;
uint32_t numNodes    = 36;
uint32_t numPackets  = 1;
//std::vector<int> present(numNodes,0);
Time live_time       = MilliSeconds(10);
//Vector destpos;
std::vector< int> vr(36,0);
//vr[0]=1;
...
bool Calc(Vector pos, Vector pos2);
                             ...
                             uint32_t pktCount, Time pktInterval, int mark )
{
	...
	if (mark == 1) {
		...
		for (uint32_t k =0; k<whole_nb[nodeId].size();k++) {
...
bool Calc(Vector pos, Vector pos2)
{
  //Ptr<MobilityModel> mob=source_nodes.Get(33)->GetObject<MobilityModel>();
  //Vector dest_pos=mob->GetPosition();
  /* double dis1=  ((pos.x-dest_pos.x)*(pos.x-dest_pos.x))+((pos.y-dest_pos.y)*(pos.y-dest_pos.y));
   double dis2=  ((pos2.x-dest_pos.x)*(pos2.x-dest_pos.x))+((pos2.y-dest_pos.y)*(pos2.y-dest_pos.y));
   */
...
{
  /*timer+=1;
  Simulator::Schedule (MilliSeconds(timer), &Broadcast,
          source_nodes, 0, MilliSeconds(interval), 1);
  std::cout<<posi.x<<" "<<posi.y<<std::endl; 
  */
  TypeId tid = TypeId::LookupByName ("ns3::UdpSocketFactory");
   Ptr<Socket> source = Socket::CreateSocket (source_nodes.Get (src), tid);
  ...
  Simulator::ScheduleWithContext(source->GetNode ()->GetId (), MilliSeconds(0), &GenerateTraffic,
                   source, packetSize, numPackets, MilliSeconds(0), 1);
        Simulator::Schedule (interPacketInterval, &Broadcast,
                   source_nodes, src, interPacketInterval, 1);
  ...
  Ipv4Address senderAddr = inetSourceAddr.GetIpv4 ();
   Simulator::Schedule (MilliSeconds(interval), &Broadcast,
                   source_nodes, src, MilliSeconds(interval), 1);
...
void Broadcast ( NodeContainer source_nodes, uint32_t sender, Time interPacketInterval, int mark)
{
  ...
  Ptr<MobilityModel> mob2=source_nodes.Get(sender)->GetObject<MobilityModel>();
  Vector source_pos=mob2->GetPosition(); 
   std::cout<<source_pos.x<<std::endl;
 	for (int n = 1; n < (int)numNodes; n++)
    {

      Ptr<MobilityModel> mob=source_nodes.Get(n)->GetObject<MobilityModel>();
      posi=mob->GetPosition();

      if(Calc(posi,source_pos) )// && present[n]==0)
      
      { // present[n]=1;
       std::cout<<"sender "<< sender << "receiver " << n<< std::endl;
      //std::cout<<"ghusa"<<std::endl;
   ...
   /*  if (mark == 2)
      {
         ...
         for (uint32_t k =0; k<whole_nb[sender].size();k++) {
...
//  Ptr<MobilityModel> mob3=source_nodes.Get(33)->GetObject<MobilityModel>();
 // destpos=mob3->GetPosition(); 
 ...
 // for (size_t i = 0; i < numNodes; i++) {
  	Simulator::Schedule (MilliSeconds(1%10), &Broadcast,
  				source_nodes, 0, MilliSeconds(interval), 1);
  //}
  uint32_t sender = 0;
  ...
  int recount = 0;
  for ( uint32_t i =0; i< whole_nb.size(); i++) {
      int reflag = 0;
      std::cout.width(5);
      std::cout << i  << ": ";
	for (uint32_t k =0; k<whole_nb[i].size();k++) {
        std::cout.width(5);
