...
pdr_set.push_back(pdr);
 ...
 e2e.push_back((elapsed.count())/count);
}
...
// }
for(uint32_t tym=15;tym<25;tym+=10)
{
...
tt=Simulator::Now ().GetSeconds ()-tt;
 ...
 {
  std::cout<<e2e[j]<<std::endl;
  avg1+=e2e[j];
 }
 avg1=avg1/(e2e.size());
 std::cout<<"Avg E2E:"<<avg1<<std::endl;
 return 0;
}
